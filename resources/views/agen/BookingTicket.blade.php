...
@section('cardsAgen')
        ...
        <div class="w-full h-1/2 p-3 flex flex-col gap-4 rounded-lg mb-5 items-center ">
            ...
            <div class="flex flex-wrap w-3/4 gap-2 justify-evenly ">
                @foreach ($seat->sortBy('No_Seat') as $dataseat)
                    @if ($dataseat->ID_Jadwal === $DataJadwal->ID_Jadwal)
                        ...
                        @if ($dataseat->Available == false)
                            <button class="seat-button" data-seat="{{ $dataseat->No_Seat }}">
                                <svg version="1.0" xmlns="http://www.w3.org/2000/svg" width="50px" height="50px"
                                    viewBox="0 0 512.000000 512.000000" preserveAspectRatio="xMidYMid meet">
                ...
                <div class="w-full h-1/2 p-3 flex flex-col rounded-lg mb-5 bg-white mt-5">
                    ...
                    <a href="#" id="data-penumpang-link"><button type="submit"
                            class="p-2 px-10 bg-black text-white rounded-xl text-md mt-3">Data
                            Penumpang</button></a>
                </div>
            </div>
        </div>
    ...
    <script>
        // Fungsi untuk memperbarui tautan "Data Penumpang" dengan data kursi yang dipilih
        function updateLink() {
            let selectedSeats = document.querySelectorAll('[fill="#D49137"]');
            let seatNumbers = [];

            // Ambil nomor kursi dari tombol kursi yang dipilih
            selectedSeats.forEach(seat => {
                let seatNumber = seat.closest('.seat-button').getAttribute('data-seat');
                seatNumbers.push(seatNumber);
            });

            // Konversi array ke dalam format JSON
            let seatJSON = JSON.stringify(seatNumbers);

            // Ambil ID Jadwal dari PHP Blade
            let ID_Jadwal = "{{ $DataJadwal->ID_Jadwal }}";

            // Perbarui tautan "Data Penumpang" dengan ID_Jadwal dan data kursi yang dipilih
            let link = document.getElementById('data-penumpang-link');
            link.href =
                "{{ route('dataPenumpang', ['ID_Jadwal' => ':ID_Jadwal', 'Seat' => ':Seat', 'Tujuan' => $Tujuan]) }}"
                .replace(':ID_Jadwal', ID_Jadwal)
                .replace(':Seat', seatJSON);
        }
        ...
        function updateHarga() {
            ...
            let jumlahOrder = parseInt(document.getElementById('jumlah-order').textContent);
            let totalHarga = jumlahOrder * parseInt(harga);
            let formattedHarga = formatRupiah(totalHarga);
            document.getElementById('hargatotal').textContent = formattedHarga;
        }
